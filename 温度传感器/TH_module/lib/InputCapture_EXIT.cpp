...
{
	static double riseTime = 0;
	static u8 cnt=0,flag=0,num=0;	
	...
	{
		mInterval = (tskmgr.Time() - riseTime)*1000000;//falling edge time
		
		#ifdef USE_USE_CAPTURE_AM2303_EXIT
			if(mNumbermdataTime >Max-2)//在下降沿的时候记录
			{
				mNumbermdataTime = 0;
			}		
			else
			{
				mdataTime[mNumbermdataTime] = mInterval;
				mNumbermdataTime++;
			}
		#endif
...
{	
	...
	if(flag==1)
	{
		if(mInterval>8) //如果中途发现了头  头为10ms高电平，所以这里定位8ms
			cnt=0; //重新存储
	   else
	   {
		  mPPM[cnt++]=mInterval; //保存数据到数组
	   }
		if(cnt>=8) //当保存了一组数据后
		{
			cnt=0;
...
//float InputCapture_EXIT::ValuePPM(u8 num)
//{
//	return mPPM[num];
//}
...
void InputCapture_EXIT::InitdataTime()
{
	mNumbermdataTime = 0;
	u8 x;
	for(x = 0;x<Max;x++)
